...
module Fastlane
  module Actions
    module SharedValues
      NODE_MODIFIED_FILES = :NODE_MODIFIED_FILES
    end
    ...
    class << self
      ...
      def add_modified_files(files)
        modified_files = lane_context[SharedValues::NODE_MODIFIED_FILES] || Set.new
        modified_files += files
        lane_context[SharedValues::NODE_MODIFIED_FILES] = modified_files
      end
    end

    class CommitNodeVersionBumpAction < Action
      def self.run(params)
        repo_path = Actions.sh('git rev-parse --show-toplevel').strip
        repo_pathname = Pathname.new(repo_path)

        package_json_path = File.expand_path(File.join('.', 'package.json'))
        package_lock_path = File.expand_path(File.join('.', 'package-lock.json'))
        # changelog_path = File.expand_path(File.join('.', 'CHANGELOG.md'))

        package_json_path_relative = Pathname.new(package_json_path).relative_path_from(repo_pathname).to_s
        package_lock_path_relative = Pathname.new(package_lock_path).relative_path_from(repo_pathname).to_s
        # changelog_path_relative = Pathname.new(changelog_path).relative_path_from(repo_pathname).to_s

        UI.user_error!("Could not find the specified Node/NPM project: #{package_json_path}") unless File.exists?(package_json_path)
        UI.user_error!("Could not find the NPM lockfile: #{package_lock_path}") unless File.exists?(package_lock_path)
        ...
        expected_changed_files << package_json_path_relative
        expected_changed_files << package_lock_path_relative
        # expected_changed_files << changelog_path_relative
      ...
      def self.output
        [
          ['NODE_MODIFIED_FILES', 'The list of paths of modified files']
        ]
      end
      ...
      class << self
        def modified_files_relative_to_repo_root(repo_root)
          return [] if Actions.lane_context[SharedValues::NODE_MODIFIED_FILES].nil?

          root_pathname = Pathname.new(repo_root)
          all_modified_files = Actions.lane_context[SharedValues::NODE_MODIFIED_FILES].map do |path|
